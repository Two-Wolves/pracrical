 ...
 * 大棚园区管理
 */
class Park extends AdminController
{
    ...
    public function index()
    {
        if ($this->request->get('type') == 'ajax') {
            ...
            return json($this->model->getParkList($page, $limit, $search));
        }
        $basic_data = [
            'title' => '大棚园区管理',
            'data'  => '',
    ...
    public function add() {
        if (!$this->request->isPost()) {
            ...
            //验证数据
            $validate = $this->validate($post, 'app\admin\validate\Park.add');
    ...
    public function edit() {
        if (!$this->request->isPost()) {
            ...
            //验证数据
            $validate = $this->validate($post, 'app\admin\validate\Park.edit');
            if (true !== $validate) return __error($validate);
    ...
    public function del() {
        ...
        //验证数据
        if (!is_array($get['id'])) {
            $validate = $this->validate($get, 'app\admin\validate\Park.del');
            if (true !== $validate) return __error($validate);
        }
    ...
    public function status() {
        ...
        //验证数据
        $validate = $this->validate($get, 'app\admin\validate\Park.status');
        if (true !== $validate) return __error($validate);
