...
class Article extends AdminController {
    ...
    public function add() {
        if (!$this->request->isPost()) {
            $basic_data = [
                ...
                'sample_tags'   => \app\admin\model\article\Tag::getSampleTags(),
            ];
            ...
            //验证数据
            $validate = $this->validate($post, 'app\admin\validate\article\Article.add');
    ...
    public function edit() {
        if (!$this->request->isPost()) {
            ...
            $tag = \app\admin\model\article\ArticleTag::where(['article_id' => $get['id']])->distinct(true)->field('tag_id')->select();
            foreach ($tag as $vo) {
                $tag_title = $tag_title . ',' . \app\admin\model\article\Tag::where(['id' => $vo['tag_id']])->value('tag_title');
            }
            $basic_data = [
                'title'         => '修改文章',
                'category_list' => \app\admin\model\article\Category::getCategoryList(),
            ...
            //验证数据
            $validate = $this->validate($post, 'app\admin\validate\article\Article.edit');
            if (true !== $validate) return __error($validate);
    ...
    public function del() {
        ...
        //验证数据
        if (!is_array($get['id'])) {
            $validate = $this->validate($get, 'app\admin\validate\article\Article.del');
            if (true !== $validate) return __error($validate);
        }
    ...
    public function status() {
        ...
        //验证数据
        $validate = $this->validate($get, 'app\admin\validate\article\Article.status');
        if (true !== $validate) return __error($validate);
