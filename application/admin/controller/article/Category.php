...
class Category extends AdminController {
    ...
    public function __construct() {
        parent::__construct();
        $this->model = new \app\admin\model\article\Category;
    }
    ...
    public function add() {
        if (!$this->request->isPost()) {
            ...
            //验证数据
            $validate = $this->validate($post, 'app\admin\validate\article\Category.add');
            if (true !== $validate) return __error($validate);
    ...
    public function edit() {
        if (!$this->request->isPost()) {
            ...
            //验证数据
            $validate = $this->validate($post, 'app\admin\validate\article\Category.edit');
            if (true !== $validate) return __error($validate);
    ...
    public function del() {
        ...
        //验证数据
        if (!is_array($get['id'])) {
            $validate = $this->validate($get, 'app\admin\validate\article\Category.del');
            if (true !== $validate) return __error($validate);
        }
    ...
    public function status() {
        ...
        //验证数据
        $validate = $this->validate($get, 'app\admin\validate\article\Category.status');
        if (true !== $validate) return __error($validate);
