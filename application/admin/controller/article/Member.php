 ...
 * 会员管理
 * Class Member
 * @package app\admin_article\controller
 */
class Member extends AdminController {
    ...
    public function __construct() {
        parent::__construct();
        $this->model = new \app\admin\model\article\Member();
    }
    ...
    public function add() {
        if (!$this->request->isPost()) {
            ...
            //验证数据
            $validate = $this->validate($post, 'app\admin\validate\article\Member.add');
    ...
    public function edit() {
        if (!$this->request->isPost()) {
            ...
            //验证数据
            $validate = $this->validate($post, 'app\admin\validate\article\Member.edit');
            if (true !== $validate) return __error($validate);
    ...
    public function reset_password() {
        if (!$this->request->isPost()) {
            ...
            //验证数据
            $validate = $this->validate($post, 'app\admin\validate\article\Member.resetPassword');
            if (true !== $validate) return __error($validate);
    ...
    public function del() {
        ...
        //验证数据
        if (!is_array($get['id'])) {
            $validate = $this->validate($get, 'app\admin\validate\article\Member.del');
            if (true !== $validate) return __error($validate);
        }
    ...
    public function status() {
        ...
        //验证数据
        $validate = $this->validate($get, 'app\admin\validate\article\Member.status');
        if (true !== $validate) return __error($validate);
