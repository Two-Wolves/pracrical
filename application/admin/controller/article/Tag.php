...
class Tag extends AdminController {
    ...
    public function __construct() {
        parent::__construct();
        $this->model = new \app\admin\model\article\Tag;
    }
    ...
    public function add() {
        if (!$this->request->isPost()) {
            ...
            //验证数据
            $validate = $this->validate($post, 'app\admin\validate\article\Tag.add');
    ...
    public function edit() {
        if (!$this->request->isPost()) {
            ...
            //验证数据
            $validate = $this->validate($post, 'app\admin\validate\article\Tag.edit');
            if (true !== $validate) return __error($validate);
    ...
    public function del() {
        ...
        //验证数据
        if (!is_array($get['id'])) {
            $validate = $this->validate($get, 'app\admin\validate\article\Tag.del');
            if (true !== $validate) return __error($validate);
        }
