...
class Article extends ModelService {
    ...
    public static function edit($update) {
        ...
        \app\admin\model\article\ArticleTag::where(['article_id' => $update['article_id']])->delete();
    ...
    protected static function __buildAddTag($article_id, $insert) {
        if (isset($insert['tag_list']) && !empty($insert['tag_list']) && !empty($article_id)) {
            ...
            foreach ($tag_list as $vo) {
                $tag_id = \app\admin\model\article\Tag::where(['tag_title' => $vo])->value('id');
                if (empty($tag_id)) {
                    $tag_id = \app\admin\model\article\Tag::insertGetId(['tag_title' => $vo]);
                }
                $save_all[] = [
            ...
            \app\admin\model\article\ArticleTag::insertAll($save_all);
        }
    }
        ...
        $data = self::where($where)->field('id,category_id,member_id,title,cover_img,describe,recommend,praise,clicks,sort,remark,status,is_open,create_at')
            ...
            ->each(function ($item, $key) {
                ...
                if ($item['member_id'] == 0) {
                    $item['username'] = 'admin';
                    $item['nickname'] = '管理员';
                    $item['head_img'] = '/static/image/face.png';
                } else {
                ...
                $item['commont_total'] = model('article.commont')->where(['article_id' => $item['id']])->count();
            });
