    ...
    $.form = new function () {
        ...
        this.imageRender = function (divId, type = 'one') {
            ...
            if (type == 'one') {
                ...
                var name = $inputParent.attr('name');
                var upload_class = $parent.attr('class');
                var upload_style = $parent.attr('style');
                var uploadDiv = document.getElementById(divId);
                var upload = $parent.data('upload');
                var upload_id_html = '<input type="hidden" name="'+ name +'" id="' + upload_id + '" value="' + url + '">';
                var upload_image_html = '<div class="' + upload_class + '" data-upload="'+upload+'" data-upload-image="more" data-upload-id="' + upload_id + '" data-upload-div="' + divId + '" style="' + upload_style + '"> </div>';
                var html = '';
                //对新的所有图片url进行重新拼接
        ...
        this.imageListen = function (id) {
            ...
            //删除图片操作
            $('.upload-icon-tip').on('click', function (e) {
                //获取操作元素对象
            ...
            success: function (layero, selectIcon) {
                ...
                if (upload_iamges != null) {
                    if (upload_type == 'one') {
                        ...
                        //获取隐藏的div
                        var upload_id_html = '<input type="hidden" name="'+ upload_id +'" id="' + upload_id + '" value="' + upload_iamges + '">';
                        //重新渲染显示层
